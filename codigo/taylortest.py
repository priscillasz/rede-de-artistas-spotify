...
taylors_related_artists = sp.artist_related_artists(
    taylor_features['artist_id'])['artists']

print('Taylor has', len(taylors_related_artists),
      'related artists. The first one is given below. \n\n')
...
# if an artist have a lower popularity, it won't be in our graph.
...
for name in artists_name_list:
    ...
    if this_artist['artist_popularity'] >= popularity_threshold:
        G.add_node(this_artist['artist_name'], **
                   this_artist, related_found=False)
    else:
        ...
        if G.nodes[x]['related_found'] == False:  # then we need to find its related artists
            ...
            for rname, rdict in zip(relateds_names, relateds):
                if rdict['artist_popularity'] >= popularity_threshold:
                    ...
                    # we add an edge between x and its related rname
                    G.add_edge(x, rname)
